...
class Dataset:
    def __init__(self, data_path, inst_labels=None):
        data = np.load(data_path, allow_pickle=True)[()]
        self.X = data['X']
        self.y = data['y']
        self.inst_labels = inst_labels
       
    def get_minibatches(self, inst_labels=None, batch_size=64, shuffle=True, drop_reminder=True, use_torch=False, device='cpu'):
        ...
        for b in range(n_batches):
            ...
            if self.inst_labels is not None:
                xbatch = self.inst_labels[xbatch]
